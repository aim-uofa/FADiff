...
UNPADDED_FEATS = [
    't', 'rot_score_scaling', 'trans_score_scaling', 't_seq', 't_struct', 'motif_aatype', 'motif_rigids_0', 'motif_masks', 'motif_torsion_angles_sin_cos', 'motif_seq_masks'
]
RIGID_FEATS = [
    'rigids_0', 'rigids_t'
]
...
MOTIF_CHAIN_FEATS = [
    'motif_aatype',  'motif_masks', 
]

MOTIF_ANGLE_FEATS = [
    'motif_torsion_angles_sin_cos',
]
...
def pad_motif_feats(motif_feats, max_len, max_bb_len, use_torch=False):
    padded_feats = {
        feat_name: pad(feat, max_len, use_torch=use_torch)
        for feat_name, feat in motif_feats.items()
        if feat_name not in MOTIF_UNPADDED_FEATS + MOTIF_RIGID_FEATS + MOTIF_ANGLE_FEATS
    }
    for feat_name in MOTIF_UNPADDED_FEATS:
        if feat_name in motif_feats:
            padded_feats[feat_name] = pad(motif_feats[feat_name] , max_bb_len, use_torch=use_torch)
    for feat_name in MOTIF_RIGID_FEATS:
        if feat_name in motif_feats:
            padded_feats[feat_name] = pad_rigid(motif_feats[feat_name], max_len)
    for feat_name in MOTIF_ANGLE_FEATS:
        padded_feats[feat_name] = pad(motif_feats[feat_name], max_len, inverse=True, use_torch=use_torch)
    return padded_feats
...
def pad_feats(raw_feats, max_len, use_torch=False):
    ...
    padded_feats['motif_seq_masks'] = pad_motif_mask(raw_feats['motif_seq_masks'], max_len)
    padded_feats['motif_position'] = pad_position_motif(raw_feats['motif_position'], max_len)
    return padded_feats

def pad_motif_mask(mask: torch.tensor, max_len: int):
    pad_length = max_len - mask.size(1)
    pad = (0, pad_length)
    x_padded = F.pad(mask, pad, value=False)
    return x_padded

def pad_position_motif(mask: torch.tensor, max_len: int):
    pad_length = max_len - mask.shape[0]
    pad = (pad_length,0)
    x_padded = F.pad(mask, pad, value=False)
    return x_padded
...
def pad(x: np.ndarray, max_len: int, pad_idx=0, use_torch=False, reverse=False, inverse=False):
    ...
    if use_torch:
        return torch.nn.functional.pad(x, tuple(itertools.chain(*pad_widths)))
...
def create_data_loader(
    ...
    print(f'Creating data loader with {prefetch_factor} prefetch_factor')
    print(f'Creating data loader with {num_workers} num_workers')
...
def rigid_transform_3D(A, B, verbose=False, use_torch=True):
