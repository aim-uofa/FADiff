...
def calc_bb_fape(gt_rigids, pred_rigids, gt_atom37, pred_atom37, node_mask):
    paired_rigid_gt = gt_rigids[...,None,None].invert_apply(gt_atom37[...,None,:, :3,:])
    paired_rigid_pred = pred_rigids[...,None,None].invert_apply(pred_atom37[...,None,:, :3,:])
    edge_mask = node_mask[..., None] * node_mask[..., None, :]

    bb_error_dist = torch.sqrt(
        torch.sum((paired_rigid_gt - paired_rigid_pred) ** 2, dim=-1) + 1e-5
    )
    # motif_error_dist = torch.clamp(motif_error_dist, min=0, max=2.0)
    bb_fape_loss = torch.sum(bb_error_dist * edge_mask[...,None], dim=(-1,-2,-3)) / (torch.sum(edge_mask, dim=(-1,-2)) + 1e-10)
    return bb_fape_loss
...
def between_residue_bond_loss_motif(
    pred_atom_positions: torch.Tensor,  # (*, N, 37/14, 3)
    pred_atom_mask: torch.Tensor,  # (*, N, 37/14)
    this_c_gt: torch.Tensor,
    next_n_gt: torch.Tensor,
    ...
    c_n_bond_length_gt = torch.sqrt(
        eps + torch.sum((this_c_gt - next_n_gt) ** 2, dim=-1)
    )

    gt_length = residue_constants.between_res_bond_length_c_n[0]
    gt_stddev = residue_constants.between_res_bond_length_stddev_c_n[0]
    c_n_bond_length_error = torch.sqrt(eps + (c_n_bond_length - gt_length) ** 2)
    c_n_bond_length_error_gt = torch.sqrt(eps + (c_n_bond_length_gt - gt_length) ** 2)
    c_n_loss_per_residue = torch.nn.functional.relu(
        c_n_bond_length_error - tolerance_factor_soft * gt_stddev
    )

    has_no_gap_mask =  (
        c_n_bond_length_error_gt < (tolerance_factor_hard * gt_stddev)
    )
