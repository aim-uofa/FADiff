...
class Experiment:
    ...
    def __init__(
        ...
        self._available_gpus = ''.join(
            [str(x) for x in GPUtil.getAvailable(maxLoad=0.7, maxMemory=0.7,
        ...
        if conf.experiment.warm_start:
            ...
            if conf.experiment.use_warm_start_schedule:
    ...
    def train_epoch(
        ...
        for train_feats in train_loader:
            ...
            if self._exp_conf.ckpt_dir is not None and (
                    ...
                    or (self._exp_conf.early_ckpt and self.trained_steps == 5)
                ):
                ...
                self._log.info(f'Start evaluation ......')
            ...
            if self._use_wandb:
                ...
                wandb_logs = {
                    'loss': loss,
                    #'motif_loss': aux_data['motif_loss'],
                    ...
                    'c_n_loss': aux_data['c_n_loss'],
                    'ca_c_n_loss': aux_data['ca_c_n_loss'],
                    'c_n_ca_loss': aux_data['c_n_ca_loss'],
                    'bb_fape': aux_data['bb_fape'],
                ...
                wandb_logs.update(eu.t_stratified_loss(
                    ...
                    du.move_to_np(aux_data['c_n_loss_batch']),
                    loss_name='c_n_loss',
                ))

                wandb_logs.update(eu.t_stratified_loss(
                    du.move_to_np(train_feats['t']),
                    du.move_to_np(aux_data['bb_fape_batch']),
                    loss_name='bb_fape',
                ))
    ...
    def eval_fn(self, eval_dir, valid_loader, device, min_t=None, num_t=None, noise_scale=1.0):
        ...
        for valid_feats, pdb_names in valid_loader:
            ...
            motif_seq_mask = du.move_to_np(valid_feats['motif_seq_masks'])
            ...
            B, motif_num, seq_len = motif_seq_mask.shape
            seq_mask = np.ones((B, seq_len), dtype=np.bool_)
            for i in range(motif_num):
                seq_mask *= (~motif_seq_mask[:,i,:])
            seq_mask = (~seq_mask)
            res_aatype = aatype * seq_mask
            ...
            for i in range(batch_size):
                ...
                unpad_gt_aatype_motif = res_aatype[i][res_mask[i]]
                ...
                saved_path = au.write_prot_to_pdb(
                    ...
                    aatype=unpad_gt_aatype_motif
                )
                ind_res = reorder(unpad_prot[:,:4,:])
                res_reorder = reorder_structure(unpad_prot, ind_res)
                saved_path_reorder = au.write_prot_to_pdb(
                    res_reorder,
                    os.path.join(
                        eval_dir,
                        f'len_{num_res}_sample_{i}_diffused_{percent_diffused:.2f}_reorder.pdb'
                    ),
                    no_indexing=True,
                    b_factors=np.tile(1 - unpad_fixed_mask[..., None], 37) * 100,
                    aatype=unpad_gt_aatype_motif
    ...
    def loss_fn(self, batch):
        ...
        # TODO: loss, TM-Score of motif, reference in inference_se3_diffusion.py
        '''
        if 'motif' in self._data_conf and 'motif_seq_masks' in batch:
            #motif_seq_masks = batch['motif_seq_masks']
            motif_aligned_rmsd = 0
            # motif_seq_masks : (BatchSize, NumMotif, SeqLen)
            # gt_trans_x0 : (BatchSize, SeqLen, 3)

            #for i in range(motif_seq_masks.shape[-1]):
            #    motif_aligned_rmsd += metrics.calc_aligned_rmsd(gt_trans_x0[motif_seq_masks[...,i:i+1].expand_as(gt_trans_x0)].view(-1, 3),
            #                                                pred_trans_x0[motif_seq_masks[...,i:i+1].expand_as(gt_trans_x0)].view(-1, 3),
            #                                                use_torch=True)

            motif_seq_masks = batch['motif_seq_masks'].transpose(1,2)
            for i in range(motif_seq_masks.shape[2]):
                motif_aligned_rmsd += metrics.calc_aligned_rmsd(gt_trans_x0[motif_seq_masks[:,:,i].unsqueeze(-1).expand_as(gt_trans_x0)].view(-1, 3),
                                                            pred_trans_x0[motif_seq_masks[:,:,i].unsqueeze(-1).expand_as(gt_trans_x0)].view(-1, 3),
                                                            use_torch=True)
            motif_aligned_rmsd = motif_aligned_rmsd / motif_seq_masks.shape[-1]
            motif_loss = motif_aligned_rmsd * self._exp_conf.motif_loss_weight
        else:
            motif_loss = torch.zeros_like(trans_loss)
        '''
        # Rotation loss
        ...
        pred_rigids = ru.Rigid.from_tensor_7(model_out['rigids'])
        ...
        bond_loss = between_residue_bond_loss_motif(model_out['atom37'], atom37_mask_motif.to(pred_atom37.device), this_c_gt, next_n_gt)
        bond_loss['c_n_loss_mean'] *= self._exp_conf.c_n_loss_weight
        bond_loss['c_n_loss_mean'] *= (batch['t'] < self._exp_conf.bb_atom_loss_t_filter)
        bond_loss['ca_c_n_loss_mean'] *= (batch['t'] < self._exp_conf.bb_atom_loss_t_filter)
        bond_loss['c_n_ca_loss_mean'] *= (batch['t'] < self._exp_conf.bb_atom_loss_t_filter)
        
        
        bb_fape = calc_bb_fape(gt_rigids, pred_rigids, gt_atom37, pred_atom37, bb_mask)
        bb_fape *= (batch['t'] < self._exp_conf.bb_atom_loss_t_filter)
        ...
        final_loss = (
            ...
            + bb_atom_loss
            + dist_mat_loss
            + bond_loss['c_n_loss_mean']
            + bond_loss['ca_c_n_loss_mean']
            + bond_loss['c_n_ca_loss_mean']
            + bb_fape
        )
        ...
        aux_data = {
            ...
            #'batch_motif_loss': motif_loss,
            'batch_bb_atom_loss': bb_atom_loss,
            'c_n_loss_batch': bond_loss['c_n_loss_mean'],
            'ca_c_n_loss_batch': bond_loss['ca_c_n_loss_mean'],
            'c_n_ca_loss_batch': bond_loss['c_n_ca_loss_mean'],
            'bb_fape_batch': bb_fape,
            ...
            #'motif_loss': normalize_loss(motif_loss),
            ...
            'c_n_loss': normalize_loss(bond_loss['c_n_loss_mean']),
            'ca_c_n_loss': normalize_loss(bond_loss['ca_c_n_loss_mean']),
            'c_n_ca_loss': normalize_loss(bond_loss['c_n_ca_loss_mean']),
            'bb_fape': normalize_loss(bb_fape),
        }
    ...
    def inference_fn_tsp(
        ...
        with torch.no_grad():
            ...
            for ind, t in enumerate(reverse_steps):
                if t > min_t:
                    sample_feats = self._set_t_feats(sample_feats, t, t_placeholder)
                    if ind < 100 and ind%10==0 and ind!=0:
                        psi_out = sample_feats['torsion_angles_sin_cos'][..., 2, :].detach().clone()
                        atom37_out, atom37_mask, _, _ = all_atom.compute_backbone(
                        ru.Rigid.from_tensor_7(sample_feats['rigids_t']), psi_out)
                        ind_res = eu.reorder(atom37_out.numpy()[0], sample_feats['motif_seq_masks'].cpu().numpy()[0])
                        sample_feats_update = eu.reorder_structure(sample_feats, ind_res)
                        sample_feats.update(sample_feats_update)
                    else:
                        ind_res = None
                    ...
                    rigids_t = self.diffuser.reverse(
                        ...
                        motif_mask=du.move_to_np(sample_feats['motif_seq_masks']),
                        rot_increase_trans=du.move_to_np(model_out['rot_increase_trans'])
                    )
                else:
                    model_out = self.model(sample_feats)
        ...
        if aux_traj:
            ...
            #ret['motif_seq_masks'] = 
        return ret, all_reindex,sample_feats['motif_seq_masks'].cpu().numpy(), sample_feats['motif_position']
    ...
    def inference_fn(
        ...
        all_trans_0_pred = []
        all_bb_0_pred = []
        sample_feats['inpainting'] = inpainting
        with torch.no_grad():
            if self._model_conf.embed.embed_self_conditioning and self_condition:
                sample_feats = self._set_t_feats(
                    sample_feats, reverse_steps[0], t_placeholder)
...
@hydra.main(version_base=None, config_path="../config", config_name="entry")
