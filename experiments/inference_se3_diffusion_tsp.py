...
class Sampler:
        ...
        for idx in range(len(self.selected_motifs)):
            ...
            for sample_length in all_sample_lengths:
                ...
                for sample_i in range(self._sample_conf.samples_per_length):
                    ...
                    import json
                    reindex_dir = os.path.join(sample_dir, 'reindex.csv')
                    df =pd.DataFrame(sample_output['reindex'])
                    df.to_csv(reindex_dir, index=None)
    ...
    def sample(self, sample_length: int, motif_dict=None, scale=None):
        ...
        # Initialize data
        # adjust mask
        if 'sz_motif' in self._inference_type:
        ...
        # Run inference
        sample_out, all_reindex, motif_seq_masks, motif_postion = self.exp.inference_fn_tsp(
            init_feats,
        ...
        for i in range(num_motif):
            ...
            for j in range(seq_len):
                if motif_seq_masks[0][i][j]:
                    seq_aatype[j] = int(init_feats['motif_aatype'][0][i][index].cpu())
                    index += 1
        res['aatype'] = seq_aatype
        res['motif_position'] = motif_postion[0].cpu().numpy()
        res['reindex'] = all_reindex
        return res
