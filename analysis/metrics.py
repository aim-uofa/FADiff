...
def calc_aligned_rmsd(pos_1, pos_2, use_torch=False):
    if use_torch:
        ...
        '''
        if torch.isnan(aligned_pos_1).any():
            print("aligned_pos_1", torch.isnan(aligned_pos_1).any())
        if torch.isnan(pos_2).any():
            print("pos_2", torch.isnan(pos_2).any())
        if torch.isnan(torch.norm(aligned_pos_1 - pos_2, dim=-1)).any():
            print("result", torch.isnan(torch.norm(aligned_pos_1 - pos_2, dim=-1)).any())
        if torch.isnan(torch.mean(torch.norm(aligned_pos_1 - pos_2, dim=-1))).any():
            print(pos_1)
            print("final", torch.isnan(torch.mean(torch.norm(aligned_pos_1 - pos_2, dim=-1))).any())
        '''
